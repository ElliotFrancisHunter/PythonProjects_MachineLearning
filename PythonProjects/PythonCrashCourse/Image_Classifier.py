...
# Create model as a sequential type so that we can add layers in order
model = Sequential()
...
# activation: relu activation for each of the operations as it tends to produce best results
# padding: 'same' adds padding to the input image to make sure that the output feature is the same size as the input
# kernel_constraint: maxnorm normalises the values in the kernel to make sure that the max layer is 3
...
# Flatten layer converts a matrix into a one dimensional array
model.add(Flatten())
...
# activation: relu because it tends to be the most efficient
model.add(Dense(units=512, activation='relu', kernel_constraint=maxnorm(3)))
# Dropout later to ignore some neurons during training which improves model reliability
...
model.fit(x=x_train, y=y_train, epochs=50, batch_size=32) # Does the training
